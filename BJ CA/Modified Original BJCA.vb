...
#End Region
    ...
    Public Sub fixforcedstrat(ByRef temp As Object)

    End Sub

    Public Sub copyeorforcedstrat(ByRef temp As Object)

    End Sub

    Public Sub computeeorforcedstrat(ByRef temp As Object)
...
#Region " General Methods "
    ...
    Private Function GetPostSplitStrat(ByVal preSplitStrat As Integer, ByVal paircard As Integer, ByVal upcard As Integer, ByVal cstrat As BJCAStrategyClass, ByVal pHand As BJCAPlayerHandClass) As Integer
        ...
        If preSplitStrat = BJCAGlobalsClass.Strat.P Then
            postSplitStrat = cstrat.StratTD(pHand.Hand.Total, pHand.Hand.Soft + 1).Strat(upcard)
        End If
                ...
                Case BJCAGlobalsClass.Strat.R
                    If paircard = 1 Then
                        If Not DSA Then
                            ...
                            End If
                        End If
                    ElseIf Not pHand.RPostallowed(upcard) Then
                        postSplitStrat = BJCAGlobalsClass.Strat.H
                    End If
                ...
                Case BJCAGlobalsClass.Strat.PD
                    If paircard = 1 Then
                        If Not DSA Then
                            ...
                            End If
                        End If
                    ElseIf pHand.DPostallowed(upcard) Then
                        postSplitStrat = BJCAGlobalsClass.Strat.D
                    Else
                        postSplitStrat = BJCAGlobalsClass.Strat.H
                    End If
                ...
                Case BJCAGlobalsClass.Strat.PR
                    If paircard = 1 Then
                        If Not DSA Then
                            ...
                            End If
                        End If
                    ElseIf pHand.RPostallowed(upcard) Then
                        postSplitStrat = BJCAGlobalsClass.Strat.R
                    Else
                        postSplitStrat = BJCAGlobalsClass.Strat.H
                    End If
...
#Region " Initialization Methods "
            ...
            If PlayerHands(index).Hand.NumCards > 1 Then
                For upcard = 1 To 10
                    If UCAllowed(upcard) Then
                        ...
                        If PlayerHands(index).DPreallowed(upcard) Or RDA Then
    ...
    Private Sub ResetForcedRulesHands(ByRef cStrat As BJCAStrategyClass)
        Dim index As Integer
        Dim upcard As Integer
    ...
    Private Sub ApplyForcedRuleHandUpcard(ByRef cStrat As BJCAStrategyClass, ByVal index As Integer, ByVal rule As BJCAForcedRulesClass, ByVal upcard As Integer)
    ...
    Private Sub ApplyForcedRulesHand(ByRef cStrat As BJCAStrategyClass, ByVal index As Integer)
        Dim rule As Integer
        Dim upcard As Integer
        ...
        For rule = 0 To ForcedTableRulesList.NumRules - 1
            ...
            If UCAllowed(upcard) And PlayerHands(index).Hand.NumCards > 1 And ForcedTableRulesList.L(rule).RuleOn And Not (ForcedTableRulesList.L(rule).UseSpecificHand And ForcedTableRulesList.L(rule).ExactMatch And index = 0) Then
        ...
        For upcard = 1 To 10
            ...
            If cStrat.HandEVs(index).SPreallowed(upcard) = False And _
                ...
                cStrat.HandEVs(index).PreForced(upcard) = False Then
            ...
            If cStrat.HandEVs(index).SPostallowed(upcard) = False And _
                ...
                cStrat.HandEVs(index).PostForced(upcard) = False Then
...
#Region " Double Methods "
                        ...
                        Else
                            ...
                            If (BBO Or OBBO) Then
                                ...
                                If upcard = 1 And Not CheckAce And CardProb(10, 0) > 0 Then
                                    UndealPCard(card)
                                    CurrentShoe.Deal(10)
                                    newEVs.DEV(upcard) += CardProb(card, 2)
    ...
    Private Function ComputeDoubleHand(ByVal index As Integer, ByVal upcard As Integer) As BJCAHandEVsClass
            ...
            If prob > 0 Then
                ...
                ElseIf PlayerHands(index).HitHand(card) Then
                    ...
                    Else
                        ...
                        'tempevs.DEV is holding the bustprob
                        bustprob += 2 * prob * tempEVs.DEV(upcard)
                        UndealPCard(card)
                    End If
                Else
        ...
        'EV(Double | OBBO) = (1 - prob(BJ)) * CEV - prob(BJ) * (1 + p(Bust))
        'EV(Double | ENHC) = (1 - prob(BJ)) * CEV - prob(BJ) * (1 + p(Bust) + (1 - p(Bust))
        'Therefore:
        'EV(Double | OBBO) = EV(Double | ENHC) + prob(BJ)*(1 - p(Bust))
        If BBO Or OBBO Then
            If upcard = 1 And Not CheckAce Then
    ...
    Private Function ComputeDoubleStratHand(ByVal currEVs As BJCAHandEVsClass, ByVal index As Integer, ByVal upcard As Integer) As BJCAHandEVsClass
        ...
        If DDR Then
            If PlayerHands(index).HandEVs.SurrEV(upcard) > newEVs.DStratEV(upcard) Then
                newEVs.DStratEV(upcard) = PlayerHands(index).HandEVs.SurrEV(upcard)
                newEVs.DStratPushEV(upcard) = 0
                ...
                If BonusRulesList.L(rule).RuleOn Then
                    ...
                    If apply Then
                        ...
                        ElseIf (upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen) Then
                            ...
                            If Not BonusRulesList.L(rule).MustWin And Not BonusRulesList.L(rule).HandContinues Then
                                If BonusRulesList.L(rule).PayoffGeneral = 0 And Not (BonusRulesList.L(rule).Upcard = upcard And BonusRulesList.L(rule).PayoffUCGeneral > 0) Then
                                    newHandEVs.StandEV(upcard) = ((1 - bjprob) * ((winEV - loseEV) + bjprob) + bjprob * tempBJPayoff)
                                    'Only need to include pushevs when not winning since otherwise adding 0
                                    newHandEVs.StandPushEV(upcard) = pushEV
                            ...
                            If prob > 0 Then
                                ...
                                If PlayerHands(index).HitHand(card) > 0 Then
                                    ...
                                    Else
                                        ...
                                        PlayerHands(index).SuitedBonusEVs.SuitedHitEV(upcard, suit) += prob * probsuit * (PlayerHands(PlayerHands(index).HitHand(card)).SuitedBonusEVs.SuitedStratEV(upcard, suit) + PlayerHands(PlayerHands(index).HitHand(card)).SuitedBonusEVs.SuitedBonusEV(upcard, suit))
                                        If (upcard = 1 Or upcard = 10) Then
...
#Region " Split Methods "
    ...
    Private Sub ComputeSplitHandProbs()
        ...
        Dim card As Integer
        Dim i As Integer
        Dim tempCheckAce As Boolean
        Dim tempCheckTen As Boolean
                            ...
                            For card = 1 To 10
                                If CardProb(card, 0) > 0 Then
                                    ...
                                    For hands = 1 To NSplitCalcs
                                        ...
                                        If BBO Or OBBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen)) Then 'things get really ugly!!
                                            'Get conditional probs of no dealer BJ
                                            tempCheckAce = CheckAce
                                            tempCheckTen = CheckTen
                                            CheckAce = True
                                            CheckTen = True
                                            SplitProbs(paircard, upcard).PBCxCards(SplitCalcHands(hands), card) = CardProb(card, upcard)
                                            CheckAce = tempCheckAce
                                            CheckTen = tempCheckTen
                                            'Now get the probs conditioned on dealer BJ
                                            SplitProbs(paircard, upcard).PBCBJ(SplitCalcHands(hands), card) = 0
                                            If upcard = 1 And Not CheckAce Then
                                                CurrentShoe.Deal(card)
                                                SplitProbs(paircard, upcard).PBCBJ(SplitCalcHands(hands), card) = CardProb(10, 0)
                                                CurrentShoe.Undeal(card)
                                                If CardProb(10, 0) > 0 Then
                                                    CurrentShoe.Deal(10)
                                                    SplitProbs(paircard, upcard).PBxCards(SplitCalcHands(hands), card) = CardProb(card, 0)
                                                    CurrentShoe.Undeal(10)
                                                Else
                                                    SplitProbs(paircard, upcard).PBxCards(SplitCalcHands(hands), card) = 0
                                                End If
                                            ElseIf upcard = 10 And Not CheckTen Then
                                                CurrentShoe.Deal(card)
                                                SplitProbs(paircard, upcard).PBCBJ(SplitCalcHands(hands), card) = CardProb(1, 0)
                                                CurrentShoe.Undeal(card)
                                                If CardProb(1, 0) > 0 Then
                                                    CurrentShoe.Deal(1)
                                                    SplitProbs(paircard, upcard).PBxCards(SplitCalcHands(hands), card) = CardProb(card, 0)
                                    ...
                                    For hands = 0 To NSplitHands
                                        If NPxHands(hands).NN > 0 Then
                                            ...
                                            If (BBO Or OBBO) And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen)) Then
                                                SplitProbs(paircard, upcard).PBCBJ(hands, card) = ConditionalValue(SplitProbs(paircard, upcard).PBCBJ(NPxHands(hands).UCH, card), SplitProbs(paircard, upcard).PBCBJ(NPxHands(hands).CH, card), SplitProbs(paircard, upcard).PxCards(NPxHands(hands).UCH, paircard))
                                                SplitProbs(paircard, upcard).PBCxCards(hands, card) = ConditionalValue(SplitProbs(paircard, upcard).PBCxCards(NPxHands(hands).UCH, card), SplitProbs(paircard, upcard).PBCxCards(NPxHands(hands).CH, card), SplitProbs(paircard, upcard).PBCxCards(NPxHands(hands).UCH, paircard))
                                            End If
                                        End If
                                    Next hands
                                ...
                                If nextIndex <> 0 Then
                                    ...
                                    For upcard = 1 To 10
                                        If Not PlayerHands(index).SplitEVs(PlayerHands(index).PairIndex(paircard), SplitCalcHands(hands)).StandDone(upcard) And UCAllowed(upcard) And CardProb(upcard, 0) > 0 Then
                                            ...
                                            If ((((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen)) And PlayerHands(index).Hand.Total = 21)) Then
                                                If upcard = 1 And Not CheckAce Then
                                ...
                                For card = 1 To 10
                                    If CurrentShoe.Cards(card) Then
                                        ...
                                        If (prob > 0) And PlayerHands(index).HitHand(card) Then
                                            Opt.HandEVs(index).SplitData(PlayerHands(index).PairIndex(paircard), hands).HitEV(upcard) += prob * Opt.HandEVs(PlayerHands(index).HitHand(card)).SplitData(PlayerHands(PlayerHands(index).HitHand(card)).PairIndex(paircard), hands).StratEV(upcard)
    ...
    Private Sub ComputeSplit(ByRef cStrat As BJCAStrategyClass, ByVal paircard As Integer, ByVal upcard As Integer)
        ...
        If index > 0 Then
            ...
            ComputeSplitEV(cStrat, paircard, upcard)
    ...
    Private Function ComputeSplitDoubleCardEV(ByVal index As Integer, ByVal upcard As Integer, ByVal depth As Integer) As BJCAHandEVsClass
        ...
        Select Case strat
            ...
            Case BJCAGlobalsClass.Strat.R
                tempEVs.Empty()
                tempEVs = ComputeSurrenderHand(CurrentShoe.Hand, BJCAGlobalsClass.Surr.LS, upcard)
    ...
    Private Sub ComputeSplitEV(ByRef cStrat As BJCAStrategyClass, ByVal paircard As Integer, ByVal upcard As Integer)
        Dim hands As Integer
        Dim card As Integer
        Dim i As Integer
        Dim EV As Double
            ...
            If CardProb(card, 0) > 0 Then
                ...
                For hands = 1 To NSplitCalcs
                    ...
                    Else
                        ...
                        Select Case Strat
                            Case BJCAGlobalsClass.Strat.S
                                If PlayerHands(index).SplitEVs(PlayerHands(index).PairIndex(paircard), SplitCalcHands(hands)).StandDone(upcard) Then
                                ...
                                If (BBO Or OBBO) And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen)) Then
                                    If EV = 0 Then
                                        cStrat.SplitStateEVs(paircard, upcard).PBCEVCards(SplitCalcHands(hands), card) = 0
                                    Else
                                        cStrat.SplitStateEVs(paircard, upcard).PBCEVCards(SplitCalcHands(hands), card) = ConditionalValue(EV, -1, SplitProbs(paircard, upcard).PBCBJ(SplitCalcHands(hands), card))
                                    End If
                                End If
                            Case BJCAGlobalsClass.Strat.D
                                EV = 0
                                    ...
                                    Else
                                        ...
                                        ElseIf Not (RDAPS Or DDRPS) Then
                                            ...
                                            Else
                                                ...
                                                EV = newEVs.StandEV(upcard) * prob
                                                CurrentShoe.Deal(upcard)
                                                PlayerHands(newIndex).SplitEVs(PlayerHands(newIndex).PairIndex(paircard), SplitCalcHands(hands)).StandEV(upcard) = EV / prob
                                        ...
                                        Else
                                            ...
                                            'DEV is holding the bust prob
                                            DBust += newEVs.DEV(upcard)
                                        End If
                                        UndealPCard(i)
                                    End If
                                    cStrat.SplitStateEVs(paircard, upcard).EVCards(SplitCalcHands(hands), card) += EV
                                ...
                                If (BBO Or OBBO) And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen)) Then
                                    If EV = 0 Then
                                        cStrat.SplitStateEVs(paircard, upcard).PBCEVCards(SplitCalcHands(hands), card) = 0
                                    Else
                                        cStrat.SplitStateEVs(paircard, upcard).PBCEVCards(SplitCalcHands(hands), card) = ConditionalValue(cStrat.SplitStateEVs(paircard, upcard).EVCards(SplitCalcHands(hands), card), -2, SplitProbs(paircard, upcard).PBCBJ(SplitCalcHands(hands), card))
                                        'Eliminate the extra loss for the unconditional EV part and add busts
                                        cStrat.SplitStateEVs(paircard, upcard).EVCards(SplitCalcHands(hands), card) = (cStrat.SplitStateEVs(paircard, upcard).PBCEVCards(SplitCalcHands(hands), card) * (1 - SplitProbs(paircard, upcard).PBCBJ(SplitCalcHands(hands), card)) - (1 + DBust) * SplitProbs(paircard, upcard).PBCBJ(SplitCalcHands(hands), card))
                                        'Define the busts
                                        cStrat.SplitStateEVs(paircard, upcard).PBCards(SplitCalcHands(hands), card) = 2 * DBust
                                    End If
                                End If
                            Case BJCAGlobalsClass.Strat.H
                                newEVs.Empty()
                                newEVs = SplitPlayerHit(cStrat, paircard, upcard, SplitCalcHands(hands), index, card, 1)
                                EV = newEVs.StandEV(upcard)
        ...
        For hands = 0 To NSplitHands
            ...
            If (BBO Or OBBO) And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen)) Then
                ...
                If NPxHands(hands).NN = 0 Then
                    For card = 1 To 10
                        'Add up the probabilities of busting given a dealer BJ
                        cStrat.SplitStateEVs(paircard, upcard).PBx(hands) = cStrat.SplitStateEVs(paircard, upcard).PBx(hands) + cStrat.SplitStateEVs(paircard, upcard).PBCards(hands, card) * SplitProbs(paircard, upcard).PBxCards(hands, card)
                        'Add them with the correct probs to get the conditional EV
                        cStrat.SplitStateEVs(paircard, upcard).PBCEVx(hands) = cStrat.SplitStateEVs(paircard, upcard).PBCEVx(hands) + cStrat.SplitStateEVs(paircard, upcard).PBCEVCards(hands, card) * SplitProbs(paircard, upcard).PBCxCards(hands, card)
                    Next card
                    cStrat.SplitStateEVs(paircard, upcard).PBN(hands) = ConditionalValue(cStrat.SplitStateEVs(paircard, upcard).PBx(hands), cStrat.SplitStateEVs(paircard, upcard).PBCards(hands, paircard), SplitProbs(paircard, upcard).PBxCards(hands, paircard))
                    'Then to get EVN, we also need the EVx of the paircard)
                    cStrat.SplitStateEVs(paircard, upcard).PBCEVN(hands) = ConditionalValue(cStrat.SplitStateEVs(paircard, upcard).PBCEVx(hands), cStrat.SplitStateEVs(paircard, upcard).PBCEVCards(hands, paircard), SplitProbs(paircard, upcard).PBCxCards(hands, paircard))
                Else
                    'Fill in the other states
                    cStrat.SplitStateEVs(paircard, upcard).PBx(hands) = ConditionalValue(cStrat.SplitStateEVs(paircard, upcard).PBx(NPxHands(hands).UCH), cStrat.SplitStateEVs(paircard, upcard).PBx(NPxHands(hands).CH), SplitProbs(paircard, upcard).PBxCards(NPxHands(hands).UCH, paircard))
                    cStrat.SplitStateEVs(paircard, upcard).PBCEVx(hands) = ConditionalValue(cStrat.SplitStateEVs(paircard, upcard).PBCEVx(NPxHands(hands).UCH), cStrat.SplitStateEVs(paircard, upcard).PBCEVx(NPxHands(hands).CH), SplitProbs(paircard, upcard).PBCxCards(NPxHands(hands).UCH, paircard))
                    cStrat.SplitStateEVs(paircard, upcard).PBN(hands) = ConditionalValue(cStrat.SplitStateEVs(paircard, upcard).PBN(NPxHands(hands).UCH), cStrat.SplitStateEVs(paircard, upcard).PBN(NPxHands(hands).CH), SplitProbs(paircard, upcard).PBxCards(hands, paircard))
                    cStrat.SplitStateEVs(paircard, upcard).PBCEVN(hands) = ConditionalValue(cStrat.SplitStateEVs(paircard, upcard).PBCEVN(NPxHands(hands).UCH), cStrat.SplitStateEVs(paircard, upcard).PBCEVN(NPxHands(hands).CH), SplitProbs(paircard, upcard).PBCxCards(hands, paircard))
                End If
            End If
        Next hands
        ...
        If (BBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
            EV = 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.Full) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.Full) * bjadjprob)
            EV = EV - (1 - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.Full) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.Full)) * bjadjprob
        ElseIf (OBBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
            'Determine the unconditional EV of the relevant hands including busts
            EV = cStrat.SplitStateEVs(paircard, upcard).EVx(BJCAGlobalsClass.Hands.Full)
            EV = EV + cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.Full) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.Full) * bjadjprob
        Else
            EV = 2 * cStrat.SplitStateEVs(paircard, upcard).EVx(BJCAGlobalsClass.Hands.Full)
        End If
        cStrat.SplitStateEVs(paircard, upcard).SPL(1) = EV
        '        cStrat.SplitStateEVs(paircard, upcard).SPL2(1) = EV
        ...
        If SPL > 1 Then
            ...
            'EV(N) + EV(N-N)
            If (BBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.Full) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.Full) * bjadjprob
                EV = EV + cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.N) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.N) * bjadjprob
                EV = EV - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.Full) * cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.N)) * bjadjprob
                '                ev2 = 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.N) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.N) * bjadjprob)
                '                ev2 = ev2 - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.N) * cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.N)) * bjadjprob
            ElseIf (OBBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = cStrat.SplitStateEVs(paircard, upcard).EVN(BJCAGlobalsClass.Hands.Full)
                EV = EV + cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.N) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.N) * bjadjprob
                '                ev2 = cStrat.SplitStateEVs(paircard, upcard).EVN(BJCAGlobalsClass.Hands.N)
                '                ev2 = ev2 + cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.N) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.N) * bjadjprob
            Else
            ...
            If SPL > 2 Then cStrat.SplitStateEVs(paircard, upcard).SPL(3) = prob * EV
            '            If SPL > 2 Then cStrat.SplitStateEVs(paircard, upcard).SPL2(3) = prob * ev2

            'Pxxx
            '3*EV(x-P)
            If (BBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = 3 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.P) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.P) * bjadjprob)
                EV = EV - (1 - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.P) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.P) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.P)) * bjadjprob
            ElseIf (OBBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = cStrat.SplitStateEVs(paircard, upcard).EVx(BJCAGlobalsClass.Hands.P)
                EV = EV + 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.P) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.P) * bjadjprob)
            Else
            ...
            cStrat.SplitStateEVs(paircard, upcard).SPL(2) = cStrat.SplitStateEVs(paircard, upcard).SPL(2) + prob * EV
            '            cStrat.SplitStateEVs(paircard, upcard).SPL2(2) = cStrat.SplitStateEVs(paircard, upcard).SPL2(2) + prob * EV
            ...
            If (BBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.Full) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.Full) * bjadjprob
                EV = EV + 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PN) * bjadjprob)
                EV = EV - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.Full) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PN)) * bjadjprob
                '                ev2 = cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.P) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.P) * bjadjprob
                '                ev2 = ev2 + 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PN) * bjadjprob)
                '                ev2 = ev2 - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.P) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PN)) * bjadjprob
            ElseIf (OBBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = cStrat.SplitStateEVs(paircard, upcard).EVN(BJCAGlobalsClass.Hands.Full)
                EV = EV + 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PN) * bjadjprob)
                '                ev2 = cStrat.SplitStateEVs(paircard, upcard).EVN(BJCAGlobalsClass.Hands.P)
                '                ev2 = ev2 + 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PN) * bjadjprob)
            Else
            ...
            cStrat.SplitStateEVs(paircard, upcard).SPL(2) = cStrat.SplitStateEVs(paircard, upcard).SPL(2) + prob * EV
            '            cStrat.SplitStateEVs(paircard, upcard).SPL2(2) = cStrat.SplitStateEVs(paircard, upcard).SPL2(2) + prob * ev2
        End If
        ...
        If SPL > 2 Then
            ...
            If (BBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.P) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.P) * bjadjprob
                EV = EV + cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PN) * bjadjprob
                EV = EV + cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PNN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PNN) * bjadjprob
                EV = EV - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.P) * cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PN) * cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PNN)) * bjadjprob
                '                ev2 = 3 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PNN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PNN) * bjadjprob)
                '                ev2 = ev2 - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PNN) * cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PNN) * cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PNN)) * bjadjprob
            ElseIf (OBBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = cStrat.SplitStateEVs(paircard, upcard).EVN(BJCAGlobalsClass.Hands.P)
                EV = EV + cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PN) * bjadjprob
                EV = EV + cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PNN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PNN) * bjadjprob
                '                ev2 = cStrat.SplitStateEVs(paircard, upcard).EVN(BJCAGlobalsClass.Hands.PNN)
                '                ev2 = ev2 + 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PNN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PNN) * bjadjprob)
            Else
            ...
            cStrat.SplitStateEVs(paircard, upcard).SPL(3) = cStrat.SplitStateEVs(paircard, upcard).SPL(3) + prob * EV
            '            cStrat.SplitStateEVs(paircard, upcard).SPL2(3) = cStrat.SplitStateEVs(paircard, upcard).SPL2(3) + prob * ev2
            ...
            If (BBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                ...
                EV = EV + cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PN) * bjadjprob
                EV = EV + cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PNN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PNN) * bjadjprob
                EV = EV - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.Full) * cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PN) * cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PNN)) * bjadjprob
                '               ev2 = 3 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PNN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PNN) * bjadjprob)
                '               ev2 = ev2 - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PNN) * cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PNN) * cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PNN)) * bjadjprob
            ElseIf (OBBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = cStrat.SplitStateEVs(paircard, upcard).EVN(BJCAGlobalsClass.Hands.Full)
            ...
            '4*EV(x)
            If (BBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = 4 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PP) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PP) * bjadjprob)
                EV = EV - (1 - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PP) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PP) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PP) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PP)) * bjadjprob
            ElseIf (OBBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = cStrat.SplitStateEVs(paircard, upcard).EVx(BJCAGlobalsClass.Hands.PP)
                EV = EV + 3 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PP) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PP) * bjadjprob)
            Else
            ...
            cStrat.SplitStateEVs(paircard, upcard).SPL(3) = cStrat.SplitStateEVs(paircard, upcard).SPL(3) + prob * EV
            '            cStrat.SplitStateEVs(paircard, upcard).SPL2(3) = cStrat.SplitStateEVs(paircard, upcard).SPL2(3) + prob * EV
            ...
            If (BBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.P) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.P) * bjadjprob
                EV = EV + 3 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PPN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN) * bjadjprob)
                EV = EV - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.P) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN)) * bjadjprob
                '                ev2 = cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PP) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PP) * bjadjprob
                '                ev2 = ev2 + 3 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PPN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN) * bjadjprob)
                '                ev2 = ev2 - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PP) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN)) * bjadjprob
            ElseIf (OBBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = cStrat.SplitStateEVs(paircard, upcard).EVN(BJCAGlobalsClass.Hands.P)
                EV = EV + 3 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PPN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN) * bjadjprob)
                '                ev2 = cStrat.SplitStateEVs(paircard, upcard).EVN(BJCAGlobalsClass.Hands.PP)
                '                ev2 = ev2 + 3 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PPN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN) * bjadjprob)
            Else
            ...
            If (BBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.Full) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.Full) * bjadjprob
                EV = EV + 3 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PPN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN) * bjadjprob)
                EV = EV - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.Full) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN)) * bjadjprob
                '                ev2 = cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PP) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PP) * bjadjprob
                '                ev2 = ev2 + 3 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PPN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN) * bjadjprob)
                '                ev2 = ev2 - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PP) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPN)) * bjadjprob
            ElseIf (OBBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
            ...
            If (BBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.P) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.P) * bjadjprob
                EV = EV + cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PN) * bjadjprob
                EV = EV + 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PPNN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPNN) * bjadjprob)
                EV = EV - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.P) * cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPNN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPNN)) * bjadjprob
                '                ev2 = 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PPN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PPN) * bjadjprob)
                '                ev2 = ev2 + 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PPNN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPNN) * bjadjprob)
                '                ev2 = ev2 - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PPN) * cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PPN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPNN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPNN)) * bjadjprob
            ElseIf (OBBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                EV = cStrat.SplitStateEVs(paircard, upcard).EVN(BJCAGlobalsClass.Hands.P)
                EV = EV + cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PN) * bjadjprob
                EV = EV + 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PPNN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPNN) * bjadjprob)
                '                ev2 = cStrat.SplitStateEVs(paircard, upcard).EVN(BJCAGlobalsClass.Hands.PPN)
                '                ev2 = ev2 + cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PPN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PPN) * bjadjprob
                '                ev2 = ev2 + 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PPNN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPNN) * bjadjprob)
            Else
            ...
            If (BBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
                ...
                EV = EV + cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PN) * bjadjprob
                EV = EV + 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PPNN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPNN) * bjadjprob)
                EV = EV - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.Full) * cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPNN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPNN)) * bjadjprob
                '                ev2 = 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVN(BJCAGlobalsClass.Hands.PPN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PPN) * bjadjprob)
                '                ev2 = ev2 + 2 * (cStrat.SplitStateEVs(paircard, upcard).PBCEVx(BJCAGlobalsClass.Hands.PPNN) * (1 - bjadjprob) - cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPNN) * bjadjprob)
                '                ev2 = ev2 - (1 - cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PPN) * cStrat.SplitStateEVs(paircard, upcard).PBN(BJCAGlobalsClass.Hands.PPN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPNN) * cStrat.SplitStateEVs(paircard, upcard).PBx(BJCAGlobalsClass.Hands.PPNN)) * bjadjprob
            ElseIf (OBBO And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen))) Then
    ...
    Private Function SplitPlayerHit(ByRef cStrat As BJCAStrategyClass, ByVal paircard As Integer, ByVal upcard As Integer, ByVal hands As Integer, ByVal oldindex As Integer, ByVal basecard As Integer, ByVal NetProb As Double) As BJCAHandEVsClass
        Dim card As Integer
        Dim i As Integer
        Dim EV As Double
        Dim DEV As Double
        ...
        For card = 1 To 10
            If CardProb(card, 0) > 0 Then
                ...
                If (BBO Or OBBO) And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen)) Then
                    If upcard = 1 And Not CheckAce Then
                        If CardProb(10, 0) > 0 Then
                            ...
                            newNetProb = NetProb * CardProb(card, 0)
                ...
                If (CurrentPHand.Total + card) > 21 And Not CurrentPHand.Soft Then  'Busted
                    EV = EV - prob
                    If (BBO Or OBBO) And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen)) Then
                        cStrat.SplitStateEVs(paircard, upcard).PBCards(hands, basecard) += newNetProb
                    End If
                Else    'Not busted
                    DealPCard(card)
                    ...
                    Select Case Strat
                        Case BJCAGlobalsClass.Strat.S   'Stand
                            If PlayerHands(index).SplitEVs(PlayerHands(index).PairIndex(paircard), hands).StandDone(upcard) Then
                                tempev = PlayerHands(index).SplitEVs(PlayerHands(index).PairIndex(paircard), hands).StandEV(upcard)
                        ...
                        Case BJCAGlobalsClass.Strat.D   'Double
                            ...
                            For i = 1 To 10 'Deal double card
                                ...
                                If dprob > 0 Then
                                    ...
                                    DEV = DEV + prob * dprob * tempev
                                    DPush = DPush + prob * dprob * temppush
                                    UndealPCard(i)
                                End If
                            Next i
                            EV = EV + 2 * DEV
                            push = push + 2 * DPush
                            If (BBO Or OBBO) And ((upcard = 1 And Not CheckAce) Or (upcard = 10 And Not CheckTen)) Then
                                cStrat.SplitStateEVs(paircard, upcard).PBCards(hands, basecard) += 2 * DBust
                            End If
                        Case BJCAGlobalsClass.Strat.H  'Hit
                            newEVs.Empty()
                            newEVs = SplitPlayerHit(cStrat, paircard, upcard, hands, index, basecard, newNetProb)
                            tempev = newEVs.StandEV(upcard)
                            temppush = newEVs.StandPushEV(upcard)
                        ...
                        Case BJCAGlobalsClass.Strat.R
                            newEVs = ComputeSurrenderHand(CurrentShoe.Hand, cStrat.HandEVs(index).RPostallowed(upcard), upcard)
                            EV = EV + prob * newEVs.SurrEV(upcard)
                    End Select
                    UndealPCard(card)
                End If  'Busted
            End If  'Currentshoe(card)
...
#Region " Strategy Methods "
    ...
    Private Sub ComputeStratHitTotal(ByRef cStrat As BJCAStrategyClass, ByVal Total As Integer, ByVal Soft As Boolean, ByVal upcard As Integer)
        ...
        index = PlayerHandTotal(Total, Soft + 1)
        Do While index
            If HandPossible(PlayerHands(index).Hand) Then
                CurrentShoe.Deal(PlayerHands(index).Hand)
                ...
                For card = 1 To 10
                    If CardProb(card, 0) > 0 Then
                        ...
                        If PlayerHands(index).HitHand(card) Then
                            ...
                            ElseIf PlayerHands(PlayerHands(index).HitHand(card)).Hand.NumCards <= cStrat.NCardsCD Then
                                Strat = cStrat.HandEVs(PlayerHands(index).HitHand(card)).EVs.Strat(upcard)
                            Else
                            ...
                            End If
                            If Strat = BJCAGlobalsClass.Strat.S Or Strat = BJCAGlobalsClass.Strat.PS Or (Strat = BJCAGlobalsClass.Strat.DS And Not cStrat.HandEVs(PlayerHands(index).HitHand(card)).DPreallowed(upcard)) Or (Strat = BJCAGlobalsClass.Strat.RS And Not cStrat.HandEVs(PlayerHands(index).HitHand(card)).RPreallowed(upcard)) Then
                                newStrat = BJCAGlobalsClass.Strat.S
                            ElseIf Strat = BJCAGlobalsClass.Strat.H Or Strat = BJCAGlobalsClass.Strat.PH Or ((Strat = BJCAGlobalsClass.Strat.D Or Strat = BJCAGlobalsClass.Strat.PD) And Not cStrat.HandEVs(PlayerHands(index).HitHand(card)).DPreallowed(upcard)) Or ((Strat = BJCAGlobalsClass.Strat.R Or Strat = BJCAGlobalsClass.Strat.PR) And Not cStrat.HandEVs(PlayerHands(index).HitHand(card)).RPreallowed(upcard)) Then
    ...
    Public Function ComputeNCardStratEVs(ByRef cStrat As BJCAStrategyClass, ByVal total As Integer, ByVal soft As Integer, ByVal upcard As Integer, ByVal nCards As Integer, ByVal orMore As Boolean, ByVal orLess As Boolean, ByVal handsUsed As Boolean) As BJCATDStratClass
        ...
        If prob > 0 Then
            ...
            If Strat <> BJCAGlobalsClass.Strat.D And Strat <> BJCAGlobalsClass.Strat.DS Then
                ...
                Do While index
                    handIncluded = ((Not handsUsed Or cStrat.HandEVs(index).HandUsed(upcard) = 1) And (PlayerHands(index).Hand.NumCards = nCards Or (orMore And PlayerHands(index).Hand.NumCards >= nCards) Or (orLess And PlayerHands(index).Hand.NumCards <= nCards And PlayerHands(index).Hand.NumCards > 1)))
                    If handIncluded And cStrat.HandEVs(index).RPreallowed(upcard) Then
                        If handsUsed Then
                            prob = PlayerHands(index).HandEVs.Prob(upcard) * cStrat.HandEVs(index).Multiplier(upcard)
                        Else
                            prob = PlayerHands(index).HandEVs.Prob(upcard)
            ...
            If handIncluded Then
                ...
                ElseIf Strat = BJCAGlobalsClass.Strat.R And cStrat.HandEVs(SplitTotalIndex(total, soft)).RPostallowed(upcard) Then
    ...
    Private Sub ComputeStrategyMaxEVTotalMinus(ByRef cStrat As BJCAStrategyClass, ByVal total As Integer, ByVal soft As Integer, ByVal upcard As Integer)
        ...
        index = PlayerHandTotal(total, soft)
        Do While index
            If PlayerHands(index).Hand.NumCards < 2 Or PlayerHands(index).Hand.NumCards <= cStrat.NCardsCD Or cStrat.HandEVs(index).PreForced(upcard) Then
                cStrat.HandEVs(index).HandUsed(upcard) = -1
            End If
            'Don't include blackjack in the strategy determination
            If PlayerHands(index).Hand.NumCards = 2 And PlayerHands(index).Hand.Total = 21 Then
                cStrat.HandEVs(index).HandUsed(upcard) = -1
            End If
        ...
        If prob = 0 Then
            ...
            index = PlayerHandTotal(total, soft)
            Do While index
                If PlayerHands(index).Hand.NumCards >= 2 And cStrat.HandEVs(index).HandUsed(upcard) = -1 And Not cStrat.HandEVs(index).PreForced(upcard) Then
                    allForced = False
                    Exit Do
                End If
                index = PlayerHands(index).NextHand
            ...
            If allForced Then
                'All the hands are forced so if the strategy is the same for all the hands
                '   for the total/upcard not including pairs then fill it in -
            ...
            Else
                ...
                index = PlayerHandTotal(total, soft)
                Do While index
                    If PlayerHands(index).Hand.NumCards >= 2 And cStrat.HandEVs(index).HandUsed(upcard) = -1 Then
                        prob = PlayerHands(index).HandEVs.Prob(upcard) * cStrat.HandEVs(index).Multiplier(upcard)
                ...
                If cStrat.StratTD(total, soft).NetProb(upcard) <> 0 Then
                    ...
                    If maxev = 0 Or ((cStrat.StratTD(total, soft).NetDProb(upcard) <> 0) And (cStrat.StratTD(total, soft).StratDEV(upcard) > maxev)) Then
                        If maxev = 0 Or cStrat.StratTD(total, soft).Strat(upcard) = BJCAGlobalsClass.Strat.H Then
                    ...
                    If maxev = 0 Or ((cStrat.StratTD(total, soft).NetSurrProb(upcard) <> 0) And (cStrat.StratTD(total, soft).StratSurrEV(upcard) >= maxev)) Then
                        If maxev = 0 Or cStrat.StratTD(total, soft).Strat(upcard) = BJCAGlobalsClass.Strat.H Or cStrat.StratTD(total, soft).Strat(upcard) = BJCAGlobalsClass.Strat.D Then
        ...
        Else
            ...
            index = PlayerHandTotal(total, soft)
            Do While index
                If cStrat.HandEVs(index).HandUsed(upcard) = 1 And cStrat.HandEVs(index).RPreallowed(upcard) And cStrat.HandEVs(index).DPreallowed(upcard) Then
                    prob = PlayerHands(index).HandEVs.Prob(upcard) * cStrat.HandEVs(index).Multiplier(upcard)
            ...
            If cStrat.StratTD(total, soft).Strat(upcard) <> BJCAGlobalsClass.Strat.D And cStrat.StratTD(total, soft).Strat(upcard) <> BJCAGlobalsClass.Strat.DS Then
                index = PlayerHandTotal(total, soft)
                Do While index
                    If cStrat.HandEVs(index).HandUsed(upcard) = 1 And cStrat.HandEVs(index).RPreallowed(upcard) Then
                        prob = PlayerHands(index).HandEVs.Prob(upcard) * cStrat.HandEVs(index).Multiplier(upcard)
                        cStrat.StratTD(total, soft).NetProb(upcard) += prob
                        cStrat.StratTD(total, soft).SStandEV(upcard) += prob * PlayerHands(index).HandEVs.StandEV(upcard)
                        cStrat.StratTD(total, soft).SHitEV(upcard) += prob * cStrat.HandEVs(index).EVs.HitEV(upcard)
    ...
    Private Sub ComputeStrategyMaxEVMinus(ByRef cStrat As BJCAStrategyClass, ByVal upcard As Integer)
        ...
        For index = 1 To NumPHands
            If PlayerHands(index).Hand.NumCards > 1 Then
                If PlayerHands(index).Hand.NumCards <= cStrat.NCardsCD Or cStrat.HandEVs(index).PreForced(upcard) Then
                    cStrat.HandEVs(index).HandUsed(upcard) = -1
                ElseIf cStrat.HandEVs(index).DPreallowed(upcard) And (cStrat.StratTD(PlayerHands(index).Hand.Total, PlayerHands(index).Hand.Soft + 1).Strat(upcard) = BJCAGlobalsClass.Strat.D Or cStrat.StratTD(PlayerHands(index).Hand.Total, PlayerHands(index).Hand.Soft + 1).Strat(upcard) = BJCAGlobalsClass.Strat.DS) Then
                    cStrat.HandEVs(index).HandUsed(upcard) = -1
                ElseIf cStrat.HandEVs(index).RPreallowed(upcard) And (cStrat.StratTD(PlayerHands(index).Hand.Total, PlayerHands(index).Hand.Soft + 1).Strat(upcard) = BJCAGlobalsClass.Strat.R Or cStrat.StratTD(PlayerHands(index).Hand.Total, PlayerHands(index).Hand.Soft + 1).Strat(upcard) = BJCAGlobalsClass.Strat.RS) Then
                    cStrat.HandEVs(index).HandUsed(upcard) = -1
                End If
                If cStrat.HandEVs(index).HandUsed(upcard) <> -1 Then
                    prob += 1
                    ...
                    If Not cStrat.StratTD(total, soft) Is Nothing Then
                        If cStrat.StratTD(total, soft).Strat(upcard) = BJCAGlobalsClass.Strat.H Or cStrat.StratTD(total, soft).Strat(upcard) = BJCAGlobalsClass.Strat.D Or cStrat.StratTD(total, soft).Strat(upcard) = BJCAGlobalsClass.Strat.R Then
                            ...
                            If cStrat.StratTD(total, soft).SHitEV(upcard) < cStrat.StratTD(total, soft).SStandEV(upcard) Then
                                If cStrat.StratTD(total, soft).Strat(upcard) = BJCAGlobalsClass.Strat.H Then
                                    cStrat.StratTD(total, soft).Strat(upcard) = BJCAGlobalsClass.Strat.S
                                ElseIf cStrat.StratTD(total, soft).Strat(upcard) = BJCAGlobalsClass.Strat.D Then
                                    cStrat.StratTD(total, soft).Strat(upcard) = BJCAGlobalsClass.Strat.DS
                                ElseIf cStrat.StratTD(total, soft).Strat(upcard) = BJCAGlobalsClass.Strat.R Then
                                    cStrat.StratTD(total, soft).Strat(upcard) = BJCAGlobalsClass.Strat.RS
                                End If
                            End If
                        End If
        ...
        For index = 1 To NumPHands
            If cStrat.HandEVs(index).HandUsed(upcard) = 1 And Not (PlayerHands(index).Hand.NumCards = 2 And PlayerHands(index).Hand.Total = 21) Then
                prob = PlayerHands(index).HandEVs.Prob(upcard) * cStrat.HandEVs(index).Multiplier(upcard)
                cStrat.StratTD(PlayerHands(index).Hand.Total, PlayerHands(index).Hand.Soft + 1).NetProb(upcard) += prob
                If cStrat.HandEVs(index).SPreallowed(upcard) Then
                    cStrat.StratTD(PlayerHands(index).Hand.Total, PlayerHands(index).Hand.Soft + 1).NetSProb(upcard) += prob
        ...
        For index = 1 To NumPHands
            If PlayerHands(index).Hand.NumCards > 1 And Not cStrat.HandEVs(index).PreForced(upcard) Then
                If PlayerHands(index).Hand.NumCards <= cStrat.NCardsCD Then
                    cStrat.HandEVs(index).EVs.Strat(upcard) = Opt.HandEVs(index).EVs.Strat(upcard)
                ElseIf PlayerHands(index).Hand.NumCards = 2 And PlayerHands(index).Hand.Total = 21 Then
                ...
                If SplitIndex(card, upcard) > 0 Then
                    ...
                    If Not cStrat.HandEVs(index).PreForced(upcard) Then
                        Select Case cStrat.HandEVs(index).EVs.Strat(upcard)
                            Case BJCAGlobalsClass.Strat.P, BJCAGlobalsClass.Strat.PS, BJCAGlobalsClass.Strat.PH, BJCAGlobalsClass.Strat.PD, BJCAGlobalsClass.Strat.PR
                                cStrat.HandEVs(index).EVs.StratEV(upcard) = cStrat.HandEVs(index).SplitEV(upcard)
                                If cStrat.HandEVs(index).SPreallowed(upcard) And PlayerHands(index).HandEVs.StandEV(upcard) > cStrat.HandEVs(index).EVs.StratEV(upcard) Then
                                    cStrat.HandEVs(index).EVs.StratEV(upcard) = PlayerHands(index).HandEVs.StandEV(upcard)
                                    cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.S
                                End If
                                If cStrat.HandEVs(index).HPreallowed(upcard) And cStrat.HandEVs(index).EVs.HitEV(upcard) > cStrat.HandEVs(index).EVs.StratEV(upcard) Then
                                    cStrat.HandEVs(index).EVs.StratEV(upcard) = cStrat.HandEVs(index).EVs.HitEV(upcard)
                                    cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.H
                                End If
                                If cStrat.HandEVs(index).DPreallowed(upcard) And PlayerHands(index).HandEVs.DEV(upcard) > cStrat.HandEVs(index).EVs.StratEV(upcard) Then
                                    If cStrat.NCardsCD < 2 Then
                                        cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.D
                                    Else
                                        If cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.H Then
                                            cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.D
                                        Else
                                            cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.DS
                                        End If
                                    End If
                                    cStrat.HandEVs(index).EVs.StratEV(upcard) = PlayerHands(index).HandEVs.DEV(upcard)
                                End If
                                If cStrat.HandEVs(index).RPreallowed(upcard) And PlayerHands(index).HandEVs.SurrEV(upcard) > cStrat.HandEVs(index).EVs.StratEV(upcard) Then
                                    If cStrat.NCardsCD < 2 Then
                                        cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.R
                                    Else
                                        If cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.H Or cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.D Then
                                            cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.R
                                        Else
                                            cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.RS
                                        End If
                                    End If
                                    cStrat.HandEVs(index).EVs.StratEV(upcard) = PlayerHands(index).HandEVs.SurrEV(upcard)
                                End If
                            Case Else
                                If cStrat.HandEVs(index).SplitEV(upcard) > cStrat.HandEVs(index).EVs.StratEV(upcard) Then
                                    If cStrat.NCardsCD < 2 Then
                                        cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.P
                                    Else
                                        Select Case cStrat.HandEVs(index).EVs.Strat(upcard)
                                            Case BJCAGlobalsClass.Strat.S
                                                cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.PS
                                            Case BJCAGlobalsClass.Strat.H
                                                cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.PH
                                            Case BJCAGlobalsClass.Strat.D, BJCAGlobalsClass.Strat.DS
                                                cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.PD
                                            Case BJCAGlobalsClass.Strat.R, BJCAGlobalsClass.Strat.RS
                                                cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.PR
                                            Case Else
                                                cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.P
                                        End Select
                                    End If
        ...
        'Now we need to go through the the 2 card hands for CD strategies and fix any strats
        '    that suggest a second play that is not possible e.g. P,S for 7,7
        'For 2 card or more strategies, the only way a 2-card hand can have a second strategy
        '    is if both the hand and strategy are possible post split 
        If cStrat.NCardsCD > 1 Then
            For card = 1 To 10
                If CardProb(card, 0) > 0 Then
                    DealPCard(card)
                    For card2 = card To 10
                        If CardProb(card2, 0) > 0 Then
                            ...
                            index = FindPlayerHand(CurrentPHand)
                            If Not cStrat.HandEVs(index).PreForced(upcard) Then
                                If (SplitIndex(card, upcard) > 0) Or SplitIndex(card2, upcard) > 0 Then
                                    'A,A was dealt with in ComputeOptSplit already
                                    'Remove second strategies if not enough cards remain

                                    'First make sure the post-split hand is even possible
                                    pairMultiplier = 0
                                    If SplitIndex(card, upcard) > 0 Then
                                        Select Case cStrat.HandEVs(SplitIndex(card, upcard)).EVs.Strat(upcard)
                                            Case BJCAGlobalsClass.Strat.P, BJCAGlobalsClass.Strat.PS, BJCAGlobalsClass.Strat.PD, BJCAGlobalsClass.Strat.PH, BJCAGlobalsClass.Strat.PR
                                        ...
                                        End Select
                                    End If
                                    If SplitIndex(card2, upcard) > 0 Then
                                        Select Case cStrat.HandEVs(SplitIndex(card2, upcard)).EVs.Strat(upcard)
                                            Case BJCAGlobalsClass.Strat.P, BJCAGlobalsClass.Strat.PS, BJCAGlobalsClass.Strat.PD, BJCAGlobalsClass.Strat.PH, BJCAGlobalsClass.Strat.PR
                                                pairMultiplier = 1
                                        End Select
                                    End If
                                    ...
                                    If pairMultiplier > 0 Then
                                        'Check to see if enough cards remain in the shoe for the
                                        '   hand post-split
                                        Select Case cStrat.HandEVs(index).EVs.Strat(upcard)
                                            Case BJCAGlobalsClass.Strat.P, BJCAGlobalsClass.Strat.PS, BJCAGlobalsClass.Strat.PD, BJCAGlobalsClass.Strat.PH, BJCAGlobalsClass.Strat.PR
                                                If card <> 1 Or (card = 1 And SMA And (HSA Or DSA Or SSA)) Then
                                                    If (CurrentShoe.Cards(card) >= (SPL - 1)) Then
                                                        CurrentShoe.Deal(card, (SPL - 1))
                                                        If CardProb(card, 0) = 0 Then
                                                            cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.P
                                                        End If
                                                        CurrentShoe.Undeal(card, (SPL - 1))
                                                    Else
                                                        cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.P
                                                    End If
                                                ElseIf card = 1 Then
                                            ...
                                            Case Else
                                                pairMultiplier = 0
                                                'Check to make sure at least on other paircard is present
                                                If SplitIndex(card, upcard) > 0 Then
                                                    Select Case cStrat.HandEVs(SplitIndex(card, upcard)).EVs.Strat(upcard)
                                                        Case BJCAGlobalsClass.Strat.P, BJCAGlobalsClass.Strat.PS, BJCAGlobalsClass.Strat.PD, BJCAGlobalsClass.Strat.PH, BJCAGlobalsClass.Strat.PR
                                                            If CardProb(card, 0) = 0 Then
                                                ...
                                                'Second strategy is not possible if not enough paircards
                                                If pairMultiplier = 0 Then
                                                    Select Case cStrat.HandEVs(index).EVs.Strat(upcard)
                                                        Case BJCAGlobalsClass.Strat.DS
                                                            cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.D
                                        ...
                                        End Select
                                    Else
                                        'No second strategies are possible
                                        Select Case cStrat.HandEVs(index).EVs.Strat(upcard)
                                            Case BJCAGlobalsClass.Strat.DS
                                                cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.D
                                            Case BJCAGlobalsClass.Strat.RS
                                                cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.R
                                            Case BJCAGlobalsClass.Strat.PS, BJCAGlobalsClass.Strat.PD, BJCAGlobalsClass.Strat.PH, BJCAGlobalsClass.Strat.PR
                                                cStrat.HandEVs(index).EVs.Strat(upcard) = BJCAGlobalsClass.Strat.P
                                        End Select
                                    End If
                                Else
                    ...
                    If CardProb(card2, 0) > 0 Then
                        ...
                        For upcard = 1 To 10
                            If UCAllowed(upcard) And CardProb(upcard, 0) > 0 Then
                                ...
                                Else
                                    'Bonus EVs are already included in the SuiteNetEV
...
#Region " Real Time Methods "
    ...
    Private Function ComputePostSplitAcesStratRT(ByVal index As Integer, ByVal upcard As Integer) As BJCAStratHandEVsClass
        ...
        If SSA Then
            ...
            End If
        Else
            newHandEVs.RAllowed(upcard) = False
        End If
